...
export const changeCurrentAccount = async (account: string, chatId: number) => {
    try {
        ...
        console.error(`Failed to change current account for chat id: ${chatId}`, err)
    }
}

export const updateLastPush = async (account: string, chatId: number, blockNumber: string, eventIndex: number) => {
    try {
        const res = await axios.post(getOffchainUrl(`/telegram/setLastPush`), {
            account,
            chatId,
            blockNumber,
            eventIndex
...
export const updateTelegramChat = async (account: string, chatId: number, push_notifs: boolean, push_feeds: boolean) => {
    try {
        const res = await axios.post(getOffchainUrl(`/telegram/updateTelegramChat`), {
            account,
            chatId,
            push_notifs,
            push_feeds
