...
export const showSettings = async (ctx: TelegrafContext) => {
    ...
    ctx.telegram.sendMessage(chatId, message, {
        parse_mode: 'HTML',
        reply_markup: settingsKeyboard(push_notifs, push_feeds)
    })
}
...
export const manageSettings = async (ctx: TelegrafContext, type: Type) => {
    ...
    await ctx.telegram.editMessageText(ctx.chat.id, messageId, '', message, {
        parse_mode: 'HTML',
        reply_markup: settingsKeyboard(updated.push_notifs, updated.push_feeds)
    })
}